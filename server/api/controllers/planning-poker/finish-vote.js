...
module.exports = {
  ...
  async fn(inputs) {
    ...
    // Idempotent behavior: if voting has already been completed or no active story,
    // return current session state instead of raising an error.
    if (!session.activeCardId || session.phase !== 'voting') {
      const nextSession = await broadcastSession(project.id, this.req, session);

      return buildStatePayload(nextSession, currentUser.id);
    }

    planningPokerSessions.reveal(session);
